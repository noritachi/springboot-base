...
public interface GroupRepository extends JpaRepository<Group, Long>, JpaSpecificationExecutor<Group> {
   ...
   @Query("SELECT DISTINCT g FROM Group g INNER JOIN Account a ON g.id = a.group.id INNER JOIN g.permissions p WHERE a.id = ?1 AND a.status = 1 AND p.status = 1 AND p.action LIKE %?2%")
